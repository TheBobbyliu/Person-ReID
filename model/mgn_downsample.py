...
class MGN(nn.Module):
    def __init__(self, args):
        ...
        for i in range(self.args.slice_p2+self.args.slice_p3+3):
            fc = nn.Conv2d(args.feats, num_classes, kernel_size=1, bias=True)
            self._init_fc(fc)
    ...
    def forward(self, x):
        ...
        for i in range(len(feats)):
            l = self.fc_module[i](feats[i]).squeeze(dim=3).squeeze(dim=2)
            featsclass.append(l)

        for i in range(len(feats)):
            feats[i] = feats[i].squeeze(dim=3).squeeze(dim=2)
