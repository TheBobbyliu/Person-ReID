...
class AMG_front(nn.Module):
    def __init__(self, args):
        super(AMG_front, self).__init__()
        ...
        self.backone_front = nn.Sequential(*list(self.backone.children())[:4])
        self.backone_end = nn.Sequential(*list(self.backone.children())[6:])

        self.dilation = nn.Sequential(
            multidilation(64, 256, 1, 1, True),
            multidilation(256, 256, 4, 1),
            multidilation(256, 256, 4, 1),
            multidilation(256, 512, 2, 2, True),
            multidilation(512, 512, 4, 1),
            multidilation(512, 512, 4, 1),
            multidilation(512, 512, 4, 1)
        )
